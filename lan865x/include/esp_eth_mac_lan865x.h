...
typedef struct {
    ...
    struct {
        ...
        esp_err_t (*read)(void *ctx, uint32_t cmd, uint32_t addr, void *data, uint32_t data_len);
        esp_err_t (*write)(void *ctx, uint32_t cmd, uint32_t addr, const void *data, uint32_t data_len);
        void *config;
    } custom_spi_driver;                    /*!< Custom SPI driver configuration, optional */
