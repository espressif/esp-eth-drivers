...
err_inv_data:
        ...
        *length = actual_length;
        ...
        ESP_LOGE(TAG, "lan865x_frame_receive timeout");
        ret = ESP_ERR_TIMEOUT;
    }
    return ret;
}
...
err:
        ...
        ESP_LOGE(TAG, "Failed to lock SPI");
        ret = ESP_ERR_TIMEOUT;
    }
    return ret;
}
...
static esp_err_t lan865x_set_reg_bits(emac_lan865x_t *emac, uint8_t mms, uint16_t addr, uint32_t bitmask)
{
    ...
    ESP_GOTO_ON_ERROR(lan865x_read_reg(emac, mms, addr, &reg_value), err, TAG, "Failed to read register MMS: %" PRIu8 ", ADDR: 0x%04" PRIx16, mms, addr);
    reg_value |= bitmask;
    ESP_GOTO_ON_ERROR(lan865x_write_reg(emac, mms, addr, reg_value), err, TAG, "Failed to write register MMS: %" PRIu8 ", ADDR: 0x%04" PRIx16, mms, addr);
err:
    return ret;
}
...
static esp_err_t lan865x_verify_id(emac_lan865x_t *emac)
{
    ...
    if (devid.model != 0x8650 && devid.model != 0x8651) {
        ESP_LOGE(TAG, "Invalid chip ID: 0x%04x", devid.model);
...
static esp_err_t emac_lan865x_start(esp_eth_mac_t *mac)
{
    ...
    // Configure MAC Network Control Register to enable Rx/Tx
...
static esp_err_t emac_lan865x_stop(esp_eth_mac_t *mac)
{
    ...
    // Configure MAC Network Control Register to disable Rx/Tx
    lan865x_mac_ncr_reg_t mac_ncr_mask = {
