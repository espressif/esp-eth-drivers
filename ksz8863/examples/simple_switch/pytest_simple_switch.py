...
def test_ksz8863_simple_switch_ping(dut: Dut) -> None:
    ...
    output = subprocess.run(['ping', param, '1', dut_ip], capture_output=True, check=False)  # noqa: S603,S607
    if 'unreachable' in str(output.stdout):
        raise RuntimeError(f'Unable to ping host device - {str(output.stdout)}')

    endnode_ip = endnode.get_interface_ip('enp3s0')
    output = subprocess.run(['ping', param, '1', endnode_ip], capture_output=True, check=False)  # noqa: S603,S607
    if 'unreachable' in str(output.stdout):
        raise RuntimeError(f'Unable to ping endnode - {str(output.stdout)}')
