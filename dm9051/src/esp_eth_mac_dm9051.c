...
#define DM9051_TX_POLL_DELAY_TICKS_DIV  (50)    // Tick divisor for 20us polling delay
#define DM9051_TX_WAIT_TIMEOUT_US       (2000)  // Transmit wait timeout (2ms)
#define DM9051_RX_READY_FLAG            (0x01)  // Expected value for ready packet
#define DM9051_RX_NSR_ERROR_THRESHOLD   (5)     // Max consecutive NSR read errors before reset
...
static esp_err_t emac_dm9051_rm_mac_filter(esp_eth_mac_t *mac, uint8_t *addr)
{
 ...
 static esp_err_t dm9051_setup_default(emac_dm9051_t *emac)
 {
    ...
    ESP_GOTO_ON_ERROR(dm9051_register_write(emac, DM9051_RCR, RCR_DIS_LONG | RCR_DIS_CRC), err, TAG, "write RCR failed");
     ...
     ESP_GOTO_ON_ERROR(dm9051_register_write(emac, DM9051_INTCKCR, 0x90), err, TAG, "write INTCKCR failed");
...
err:
 ...
 static esp_err_t emac_dm9051_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
 {
     ...
     do {
         ...
         if(reg_nsr) 
             ...
             vTaskDelay((pdMS_TO_TICKS(1)/DM9051_TX_POLL_DELAY_TICKS_DIV));
     } while((esp_timer_get_time() - wait_time) < DM9051_TX_WAIT_TIMEOUT_US);
     ...
     do {
         *size = 0;
         ...
         if (reg_nsr & NSR_RXRDY) {
             ...
             ESP_GOTO_ON_ERROR(dm9051_register_read(emac, DM9051_MRCMDX1, &rxbyte), err, TAG, "read MRCMDX1 failed");
 
             if(DM9051_RX_READY_FLAG != rxbyte)
             {
                 if((rxbyte > DM9051_RX_READY_FLAG) || (emac->rx_nsr_cnt++ > DM9051_RX_NSR_ERROR_THRESHOLD))
                 {
                     ESP_GOTO_ON_ERROR(dm9051_flush_recv_queue(emac), err, TAG, "flush rx queue failed");
                     ESP_GOTO_ON_FALSE(false, ESP_FAIL, err, TAG, "unexpected rx flag (0x%" PRIx8 "), reset rx fifo pointer , rx_nsr_cnt %d", rxbyte, emac->rx_nsr_cnt);
                 }
                 return ret;
             }
             emac->rx_nsr_cnt = 0;
 ...
 static void emac_dm9051_task(void *arg)
 {
     ...
     while (1) {
         ...
         dm9051_register_read(emac, DM9051_NSR, &reg_nsr);       
         /* packet received */
         if (reg_nsr & NSR_RXRDY) {
             do {
                 uint32_t buf_len;
