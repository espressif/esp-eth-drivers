...
static esp_err_t emac_w5500_add_mac_filter(esp_eth_mac_t *mac, uint8_t *addr)
{
    ...
    if (addr[0] == 0x01 && addr[1] == 0x00 && addr[2] == 0x5e) {
        ...
        emac_wiznet_set_mcast_v4_cnt(emac, emac_wiznet_get_mcast_v4_cnt(emac) + 1);
...
static esp_err_t emac_w5500_del_mac_filter(esp_eth_mac_t *mac, uint8_t *addr)
{
    ...
    if (addr[0] == 0x01 && addr[1] == 0x00 && addr[2] == 0x5e && v4_cnt > 0) {
        v4_cnt--;
...
esp_eth_mac_t *esp_eth_mac_new_w5500(const eth_w5500_config_t *w5500_config, const eth_mac_config_t *mac_config)
{
    ...
    emac = emac_wiznet_new((const eth_wiznet_config_t *)w5500_config,
                           mac_config,
                           &w5500_ops,
                           TAG,
                           "w5500_tsk");
    ...
    mac->add_mac_filter = emac_w5500_add_mac_filter;
    mac->rm_mac_filter = emac_w5500_del_mac_filter;
    mac->set_all_multicast = emac_w5500_set_all_multicast;
