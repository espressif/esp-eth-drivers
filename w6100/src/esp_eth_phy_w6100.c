...
#define W6100_WAIT_FOR_RESET_MS (2) // datasheet §8.4.1 TPRST min 0.6 ms after PHYCR1[RST]
...
{
    if (cmd != ESP_ETH_PHY_AUTONEGO_RESTART) {
        return phy_wiznet_autonego_ctrl(phy, cmd, autonego_en_stat);
    }
    ...
    ESP_GOTO_ON_ERROR(w6100_is_autoneg_enabled(w6100, &autoneg_enabled), err, TAG, "get autoneg status failed");
    ESP_GOTO_ON_FALSE(autoneg_enabled, ESP_ERR_INVALID_STATE, err, TAG, "auto negotiation is disabled");

    /* PHYCR0 must be written after reset; straps may otherwise leave MODE=7. */
    phy_wiznet_set_link_down(w6100);
    ESP_GOTO_ON_ERROR(w6100_phy_reset(phy), err, TAG, "reset PHY failed");
    ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, addr, W6100_REG_PHYLCKR, W6100_PHYLCKR_UNLOCK), err, TAG, "unlock PHY failed");
    ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, addr, W6100_REG_PHYCR0, W6100_OP_MODE_AUTO), err, TAG, "write PHYCR0 failed");
    vTaskDelay(pdMS_TO_TICKS(W6100_WAIT_FOR_RESET_MS));
...
static esp_err_t w6100_pwrctl(esp_eth_phy_t *phy, bool enable)
{
    ...
    if (enable) {
        /* Strap pins may force MODE=7 (10BT half) on reset; override to AUTO on
         * power-up only, so set_speed/set_duplex modes survive phy_reset(). */
        uint32_t phycr0 = W6100_OP_MODE_AUTO;
        ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, addr, W6100_REG_PHYCR0, phycr0),
                          err, TAG, "write PHYCR0 (force AUTO) failed");
        vTaskDelay(pdMS_TO_TICKS(W6100_WAIT_FOR_RESET_MS));
    }
    return ESP_OK;
err:
...
esp_eth_phy_t *esp_eth_phy_new_w6100(const eth_phy_config_t *config)
{
    ...
    phy_wiznet_config_t wiznet_config = {
        ...
        .pwrctl = w6100_pwrctl,
        .autonego_ctrl = w6100_autonego_ctrl,
    };
