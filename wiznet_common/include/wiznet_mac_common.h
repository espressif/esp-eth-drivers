...
typedef struct {
    ...
    /* Socket 0 registers (pre-computed addresses) - used by TX/RX code */
    uint32_t reg_sock_cr;           /*!< Socket command register */
    uint32_t reg_sock_sr;           /*!< Socket status register */
    ...
    uint32_t reg_sock_rx_wr;        /*!< Socket RX write pointer register (chip-managed) */
    ...
    /* Multicast / MAC filter callbacks (NULL = not supported, only wired on IDF >= 5.5) */
    ...
    /* Size of chip-specific context data, allocated by emac_wiznet_new() and
       reachable via emac_wiznet_get_context(). Set to 0 if no context is needed. */
    size_t context_size;
} wiznet_chip_ops_t;
...
/** Socket status values (identical on W5500 and W6100, datasheet §4.2.7 Sn_SR) */
#define WIZNET_SOCK_SR_CLOSED  (0x00)
#define WIZNET_SOCK_SR_MACRAW  (0x42)

/** Upper-bound guard while polling Sn_CR / Sn_SR / Sn_IR (not a chip timing constant) */
#define WIZNET_SOCK_CMD_GUARD_MS (1000)
 ...
 *
 * Chip-specific configs inherits this struct as their `base` member.
 */
typedef struct {
 ...
 *
 * @param wiznet_config WIZnet configuration (pass `&chip_config->base`)
...
/**
 * @brief Get pointer to chip-specific context data
 *
 * The block is `calloc`-zeroed at construction and freed when the EMAC is
 * destroyed. Its size is taken from `wiznet_chip_ops_t::context_size`.
 *
 * @param emac WIZnet EMAC instance
 * @return Pointer to chip-specific context data, or NULL if the chip declared `context_size == 0`
 */
void *emac_wiznet_get_context(emac_wiznet_t *emac);
