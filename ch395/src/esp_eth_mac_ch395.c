...
typedef struct {
    ...
    esp_err_t (*io)(void *spi_ctx, uint8_t cmd, uint8_t *tx_data,
...
typedef struct {
    void *ctx;
    void *(*init)(const void *uart_config);
    esp_err_t (*deinit)(void *uart_ctx);
    esp_err_t (*io)(void *uart_ctx, uint8_t cmd, uint8_t *tx_data,
                    uint32_t tx_len, uint8_t *rx_data, uint32_t rx_len);
    esp_err_t (*set_baudrate)(void *uart_ctx, uint32_t baudrate);
    esp_err_t (*get_baudrate)(void *uart_ctx, uint32_t *baudrate);
} eth_uart_driver_t;
#endif
...
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_driver_t spi;
#endif

#if CONFIG_ETH_CH395_INTERFACE_UART
    eth_uart_driver_t uart;
#endif
    int int_gpio_num;
...
static void *ch395_spi_init(const void *config)
{
    ...
    spi_devcfg = ch395_config->spi_devcfg;

    ESP_GOTO_ON_FALSE(
        ch395_config->spi_cs_gpio_num >= 0 && spi_devcfg->spics_io_num < 0,
        NULL, err, TAG, "driver cannot directly control the CS pin of spi bus");

    gpio_config_t gpio_cfg = {.pin_bit_mask =
                                  1ull << (ch395_config->spi_cs_gpio_num),
                              .mode = GPIO_MODE_OUTPUT,
                              .intr_type = GPIO_INTR_DISABLE
                             };
    gpio_config(&gpio_cfg);
    ESP_GOTO_ON_FALSE(gpio_set_level(ch395_config->spi_cs_gpio_num, 1)
                      == ESP_OK,
                      NULL, err, TAG, "cannot init CS pin");
    spi->cs_pin = ch395_config->spi_cs_gpio_num;

    ESP_GOTO_ON_FALSE(
        spi_devcfg->command_bits == 0 && spi_devcfg->address_bits == 0, NULL,
        err, TAG, "incorrect SPI frame format (command_bits/address_bits)");
    ESP_GOTO_ON_FALSE(
        spi_bus_add_device(ch395_config->spi_host_id, spi_devcfg, &spi->hdl)
        == ESP_OK,
        NULL, err, TAG, "adding device to SPI host #%d failed",
        ch395_config->spi_host_id + 1);
    /* create mutex */
...
static void *ch395_uart_init(const void *config)
{
    ...
    uart_devcfg = *(ch395_config->uart_devcfg);

    ESP_GOTO_ON_FALSE(
        uart_driver_install(ch395_config->uart_port_id, 2048, 0, 0, NULL, 0)
        == ESP_OK,
    ...
    ESP_GOTO_ON_FALSE(
        uart_param_config(ch395_config->uart_port_id, &uart_devcfg) == ESP_OK,
        NULL, err, TAG, "uart param init failed");
    ESP_GOTO_ON_FALSE(uart_set_pin(ch395_config->uart_port_id,
                                   ch395_config->uart_tx_gpio_num,
                                   ch395_config->uart_rx_gpio_num, -1, -1)
                      == ESP_OK,
                      NULL, err, TAG, "uart pin set failed");

    uart->port = ch395_config->uart_port_id;
...
static esp_err_t ch395_uart_set_baudrate(void *uart_ctx, uint32_t baudrate)
{
    esp_err_t ret = ESP_OK;
    eth_uart_ctx_t *uart = (eth_uart_ctx_t *)uart_ctx;
...
static esp_err_t ch395_uart_get_baudrate(void *uart_ctx, uint32_t *baudrate)
{
    esp_err_t ret = ESP_OK;
    eth_uart_ctx_t *uart = (eth_uart_ctx_t *)uart_ctx;
...
static esp_err_t ch395_exchange_data(emac_ch395_t *emac, uint8_t cmd,
                                     uint8_t *tx_data, uint32_t tx_len,
                                     uint8_t *rx_data, uint32_t rx_len)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    return ch395_spi_io(emac->spi.ctx, cmd, tx_data, tx_len, rx_data, rx_len);
#endif

#if CONFIG_ETH_CH395_INTERFACE_UART
    return ch395_uart_io(emac->uart.ctx, cmd, tx_data, tx_len, rx_data, rx_len);
#endif
}

static esp_err_t ch395_get_phy_status(emac_ch395_t *emac, uint8_t *sta)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_ctx_t *spi = (eth_spi_ctx_t *)(emac->spi.ctx);
    if (!ch395_spi_lock(spi)) {
        return ESP_ERR_TIMEOUT;
    }
    ch395_spi_start(spi);
...
#if CONFIG_ETH_CH395_INTERFACE_UART
    eth_uart_ctx_t *uart = (eth_uart_ctx_t *)(emac->uart.ctx);
...
static esp_err_t ch395_set_phy_mode(emac_ch395_t *emac, uint8_t mode)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_ctx_t *spi = (eth_spi_ctx_t *)(emac->spi.ctx);
    if (!ch395_spi_lock(spi)) {
...
static esp_err_t ch395_tx_buffer(emac_ch395_t *emac, uint8_t *data,
                                 uint32_t len)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_ctx_t *spi = (eth_spi_ctx_t *)(emac->spi.ctx);
...
static esp_err_t ch395_rx_buffer(emac_ch395_t *emac, uint8_t *data,
                                 uint32_t len)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_ctx_t *spi = (eth_spi_ctx_t *)(emac->spi.ctx);
...
static esp_err_t ch395_get_rx_size(emac_ch395_t *emac, uint32_t *len)
{
#if CONFIG_ETH_CH395_INTERFACE_SPI
    eth_spi_ctx_t *spi = (eth_spi_ctx_t *)(emac->spi.ctx);
...
static esp_err_t ch395_cmd_pending_finish(emac_ch395_t *emac)
{
    ...
    while (wait_cnt--) {
        if (ch395_exchange_data(emac, CMD01_GET_CMD_STATUS, NULL, 0, &status, 1)
                != ESP_OK) {
            return ESP_FAIL;
        }
...
static esp_err_t ch395_set_baudrate(emac_ch395_t *emac, uint32_t baudrate)
{
    ...
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD31_SET_BAUDRATE, params, 3, NULL, 0), err,
...
static esp_err_t ch395_self_check(emac_ch395_t *emac)
{
    ...
    uint8_t odata;
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD11_CHECK_EXIST, &idata, 1, &odata, 1), err,
        TAG, "self check failed");
...
static esp_err_t ch395_strict_self_check(emac_ch395_t *emac)
{
    ...
    for (; idata < 0xFF; idata++) {
        ESP_GOTO_ON_ERROR(
            ch395_exchange_data(emac, CMD11_CHECK_EXIST, &idata, 1, &odata, 1),
...
static esp_err_t ch395_get_mac_addr(emac_ch395_t *emac)
{
    ...
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD06_GET_MAC_ADDR, NULL, 0, emac->addr, 6),
...
static esp_err_t ch395_verify_id(emac_ch395_t *emac)
{
    ...
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD01_GET_IC_VER, NULL, 0, &version, 1), err,
...
static esp_err_t ch395_setup_default(emac_ch395_t *emac)
{
    ...
    /* Disable PING*/
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD10_PING_ENABLE, &param, 1, NULL, 0), err,
        TAG, "disable PING failed");

    /* Disable PPPOE*/
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD10_PPPOE_ENABLE, &param, 1, NULL, 0), err,
        TAG, "disable PPPOE failed");
    vTaskDelay(pdMS_TO_TICKS(20));

    /* Disable DHCP*/
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD10_DHCP_ENABLE, &param, 1, NULL, 0), err,
    ...
    if (emac->version >= 0x46) {
        ...
        ESP_GOTO_ON_ERROR(ch395_exchange_data(emac, CMD40_SET_FUN_PARA,
                                              fun_params, 4, NULL, 0),
...
static esp_err_t emac_ch395_start(esp_eth_mac_t *mac)
{
    ...
    ESP_GOTO_ON_ERROR(ch395_exchange_data(emac, CMD20_SET_PROTO_TYPE_SN,
    ...
    ESP_GOTO_ON_ERROR(ch395_exchange_data(emac, CMD90_SET_MAC_FILT,
                                          macfil_params, 9, NULL, 0),
                      err, TAG, "setting MAC Filter failed");
    /* open SOCK0 */
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD1W_OPEN_SOCKET_SN, &sock_num, 1, NULL, 0),
...
static esp_err_t emac_ch395_stop(esp_eth_mac_t *mac)
{
    ...
    /* close SOCK0 */
    ESP_GOTO_ON_ERROR(
        ch395_exchange_data(emac, CMD1W_CLOSE_SOCKET_SN, &sock_num, 1, NULL, 0),
        ...
        uint32_t reg_value)
{
    ...
    ESP_GOTO_ON_ERROR(ch395_set_phy_mode(emac, (uint8_t)reg_value), err, TAG,
        ...
        uint32_t *reg_value)
{
    ...
    ESP_GOTO_ON_ERROR(ch395_get_phy_status(emac, (uint8_t *)reg_value), err,
...
static esp_err_t emac_ch395_flush_recv_frame(emac_ch395_t *emac)
{
    ...
    uint8_t sock_num = 0;
    ESP_GOTO_ON_ERROR(ch395_exchange_data(emac, CMD10_CLEAR_RECV_BUF_SN,
                                          &sock_num, 1, NULL, 1),
                                     ...
                                     uint32_t length)
{
    ...
    ESP_GOTO_ON_ERROR(ch395_tx_buffer(emac, buf, length), err, TAG,
                      "write frame failed");
                                    ...
                                    uint32_t *length)
{
    ...
    ESP_GOTO_ON_ERROR(ch395_get_rx_size(emac, length), err, TAG,
    ...
    ESP_GOTO_ON_ERROR(ch395_rx_buffer(emac, buf, *length), err, TAG,
                      "receive frame failed");
    return ret;

err:
    *length = 0;
    ch395_rx_buffer(emac, buf, 1);
...
static void emac_ch395_task(void *arg)
{
    ...
    while (1) {
        ...
        ch395_exchange_data(emac, CMD01_GET_GLOB_INT_STATUS, NULL, 0, &g_intr,
                            1);
        if (!(g_intr & GINT_STAT_SOCK0)) {
            continue;
        }

        ch395_exchange_data(emac, CMD11_GET_INT_STATUS_SN, &sock_num, 1,
                            &s_intr, 1);
...
static esp_err_t emac_ch395_init(esp_eth_mac_t *mac)
{
    esp_err_t ret = ESP_OK;
    emac_ch395_t *emac = __containerof(mac, emac_ch395_t, parent);
    ...
    if (ch395_self_check(emac) != ESP_OK) {
        uint32_t baudrate;
        emac->uart.get_baudrate(emac->uart.ctx, &baudrate);
        ESP_LOGW(
            TAG,
        ...
        emac->uart.set_baudrate(emac->uart.ctx, 9600);
        ...
        emac->uart.set_baudrate(emac->uart.ctx, baudrate);
        ESP_GOTO_ON_ERROR(
...
static esp_err_t emac_ch395_del(esp_eth_mac_t *mac)
{
    emac_ch395_t *emac = __containerof(mac, emac_ch395_t, parent);
    if (emac->poll_timer) {
...
#if CONFIG_ETH_CH395_INTERFACE_UART
    emac->uart.deinit(emac->uart.ctx);
#endif
                                     ...
                                     const eth_mac_config_t *mac_config)
{
    ...
    ESP_GOTO_ON_FALSE(ch395_config && mac_config, NULL, err, TAG,
                      "invalid argument");
    ESP_GOTO_ON_FALSE(
        (ch395_config->int_gpio_num >= 0) != (ch395_config->poll_period_ms > 0),
        NULL, err, TAG, "invalid configuration argument combination");
    ...
    emac->spi.init = ch395_spi_init;
    emac->spi.deinit = ch395_spi_deinit;
    emac->spi.io = ch395_spi_io;
    /* SPI device init */
    ESP_GOTO_ON_FALSE((emac->spi.ctx = emac->spi.init(ch395_config)) != NULL,
                      NULL, err, TAG, "SPI initialization failed");
#endif

#if CONFIG_ETH_CH395_INTERFACE_UART
    ESP_LOGD(TAG, "Using default UART Driver");
    emac->uart.init = ch395_uart_init;
    emac->uart.deinit = ch395_uart_deinit;
    emac->uart.io = ch395_uart_io;
    emac->uart.set_baudrate = ch395_uart_set_baudrate;
    emac->uart.get_baudrate = ch395_uart_get_baudrate;
    /* UART device init */
    ESP_GOTO_ON_FALSE((emac->uart.ctx = emac->uart.init(ch395_config)) != NULL,
