...
static esp_err_t spi_bus_init(void)
{
    ...
    ESP_GOTO_ON_ERROR(spi_bus_initialize(CONFIG_IPERF_ETH_SPI_HOST, &buscfg, SPI_DMA_DISABLED),
...
void basic_init(esp_eth_handle_t *handle)
{
    ...
    eth_ch395_config_t ch395_config = ETH_CH395_DEFAULT_CONFIG(CONFIG_IPERF_ETH_SPI_HOST, &spi_devcfg);
    ch395_config.int_gpio_num = CONFIG_IPERF_ETH_INT_GPIO;
    ch395_config.spi_cs_gpio_num = CONFIG_IPERF_ETH_SPI_CS_GPIO;
#endif

#if CONFIG_ETH_CH395_INTERFACE_UART
    uart_config_t uart_devcfg = {
        .baud_rate = CONFIG_IPERF_ETH_UART_BAUDRATE,
    ...
    eth_ch395_config_t ch395_config = ETH_CH395_DEFAULT_CONFIG(CONFIG_IPERF_ETH_UART_PORT, &uart_devcfg);
    ch395_config.uart_tx_gpio_num = CONFIG_IPERF_ETH_UART_TX_GPIO;
    ch395_config.uart_rx_gpio_num = CONFIG_IPERF_ETH_UART_RX_GPIO;
    ch395_config.int_gpio_num = CONFIG_IPERF_ETH_INT_GPIO;
#endif

#if CONFIG_IPERF_ETH_INT_GPIO < 0
    ch395_config.poll_period_ms = CONFIG_IPERF_ETH_POLLING_MS_VAL;
#endif
